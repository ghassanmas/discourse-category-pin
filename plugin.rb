    ...
    if SiteSetting.category_pin_id?
        if topic.category_id?
            if topic.category_id == SiteSetting.category_pin_id
              ...
              topics_pinned = Topic.where(category:SiteSetting.category_pin_id).where.not(pinned_at:nil)
              topics_pinned.each do | pinned_topic |
                pinned_topic.update_pinned(false)
              end
               # Pinngin the current topic
               topic.update_pinned(true,gloabl=true, pinned_until=SiteSetting.category_pin_duration.minutes.from_now.to_s)
            end
        end
    end     
  end
